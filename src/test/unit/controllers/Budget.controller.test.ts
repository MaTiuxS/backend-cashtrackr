...
describe("BudgetController.getAll", () => {
  ...
  it("should retrieve 0 budgets for user with ID 8", async () => {
    const req = createRequest({
      method: "GET",
      url: "/api/budgets",
      user: { id: 8 },
  ...
  it("should handle errors when fetching budgets", async () => {
    ...
    expect(res._getJSONData()).toStrictEqual({
      error: "Hubo un error al listar los budget",
    });
  });
});
...
describe("BudgetController.create", () => {
  ...
  it("should handle budget creation error", async () => {
    ...
    expect(res.statusCode).toBe(500);
    expect(data).toEqual({ error: "Hubo un error al crear" });
...
describe("BudgetController.getById", () => {
  ...
  it("should return a budget with ID 10 and 0 expenses", async () => {
    const req = createRequest({
      method: "POST",
      url: "/api/budgets/:budgetId",
      budget: { id: 3 },
...
describe("BudgetController.updated", () => {
  it("should update the budget and return a succes message", async () => {
    ...
    const res = createResponse();
    await BudgetController.updated(req, res);
    const data = res._getJSONData();
